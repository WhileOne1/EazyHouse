...
`; 
const POST_RGBS = gql`
mutation editRgb($id: ID!, $name: String!) {
  editRgb(id: $id,name: $name) {
    id
  }
}
`; 
          ...
          <TableBody>
            ...
            <TableRow>
                   ...
                   <TableCell className={classes.cellMain} align="right"> Stan</TableCell>
                      ...
                      {(data.rgbs).map(device => (
                          <TableRow className={classes.row} key={device.id}>
                              <TableCell className={classes.cell} align="right">{device.id}</TableCell>
                              <TableCell className={classes.cell} align="right"><Popup trigger={<button className={classes.button}> {device.name}</button>} position="right center">
                                  <div> <Mutation mutation={POST_RGBS} >
                                      {(editRgb, { data }) => (
                                          <div>
                                              ...
                                              <form
                                                  onSubmit={e => {
                                                      e.preventDefault();
                                                      editRgb({ variables: { id: device.id, name: input.value } });
                              ...
                              <TableCell className={classes.cell} align="right"> <SwitchComponent isItOn={device.isOn} switchid={device.id}
                                  handleToggle={(e) => {
                                      e.preventDefault();
                                      socket.emit('change-rgb-value', { id1: device.id, isOn: !device.isOn })
                                  }} />
                                  <form>
                                      <div className="radio">
                                          <label>
                                              <input type="radio" value="Red" checked={device.color === 'Red'} onChange={socket.emit('change-rgb-color', { id: device.id, color: 'Red' })} />
                                              Red
                                          </label>
                                      </div>
                                      <div className="radio">
                                          <label>
                                              <input type="radio" value="Green" checked={device.color === 'Green'} onChange={socket.emit('change-rgb-color', { id: device.id, color: 'Green' })} />
                                              Green
                                           </label>
                                      </div>
                                      <div className="radio">
                                          <label>
                                              <input type="radio" value="Blue" checked={device.color === 'Blue'} onChange={socket.emit('change-rgb-color', { id: device.id, color: 'Blue' })} />
                                              Blue
                                          </label>
                                      </div>
                                  </form>
